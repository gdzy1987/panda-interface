...
import (
	...
	"github.com/XM-GO/panda-interface/tool/pkg/base"
...
var CmdClient = &cobra.Command{
	...
	Long:  "Generate the proto client code. Example: panda-tool proto client helloworld.proto",
	Run:   run,
}
...
func run(cmd *cobra.Command, args []string) {
	...
	if err = look("protoc-gen-go", "protoc-gen-go-grpc", "protoc-gen-go-http", "protoc-gen-go-errors", "protoc-gen-validate"); err != nil {
		// update the plugins
		cmd := exec.Command("panda-tool", "upgrade")
...
func generate(proto string, args []string) error {
	...
	inputExt := []string{
		"--proto_path=" + base.TemplateMod(),
		"--proto_path=" + filepath.Join(base.TemplateMod(), "third_party"),
